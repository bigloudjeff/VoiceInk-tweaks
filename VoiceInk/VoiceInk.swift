...
struct VoiceInkApp: App {
    ...
    init() {
        ...
        PromptFileManager.migrateFromUserDefaults()
        ...
        AppServiceLocator.shared.configure(whisperState: whisperState, enhancementService: enhancementService, modelContainer: container, menuBarManager: menuBarManager)
    ...
    private static func createPersistentContainer(schema: Schema, logger: Logger) -> ModelContainer? {
        do {
            ...
            let dictionarySchema = Schema([VocabularyWord.self, WordReplacement.self, VocabularySuggestion.self])
            #if LOCAL_BUILD
