...
class PolarService {
 ...
 logger.notice(" License validation failed [HTTP \(httpResponse.statusCode)]: \(errorMsg, privacy: .private)")
 ...
 if !(200...299).contains(httpResponse.statusCode) {
 let errorMsg = String(data: data, encoding: .utf8) ?? "Unknown error"
 logger.notice(" License activation failed [HTTP \(httpResponse.statusCode)]: \(errorMsg, privacy: .private)")
 ...
 if !(200...299).contains(httpResponse.statusCode) {
 let errorMsg = String(data: data, encoding: .utf8) ?? "Unknown error"
 logger.notice(" License validation with activation failed [HTTP \(httpResponse.statusCode)]: \(errorMsg, privacy: .private)")
 throw LicenseError.validationFailed(errorMsg)
 }
 }
