...
class LocalTranscriptionService: TranscriptionService {
 ...
 private weak var contextProvider: (any WhisperContextProvider)?

 init(modelsDirectory: URL, contextProvider: (any WhisperContextProvider)? = nil) {
 ...
 let currentModel = await provider.currentTranscriptionModel,
 ...
 let resolvedURL: URL? = await contextProvider?.availableModels.first(where: { $0.name == model.name })?.url
