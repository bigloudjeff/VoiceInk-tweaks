...
final class CoreAudioRecorder {
 ...
 // Audio metering (thread-safe, lock-free for real-time audio callback)
 private struct MeterState {
 var averagePower: Float = -160.0
 var peakPower: Float = -160.0
 }
 private let meterState = OSAllocatedUnfairLock(initialState: MeterState())

 var averagePower: Float {
 meterState.withLock { $0.averagePower }
 }

 var peakPower: Float {
 meterState.withLock { $0.peakPower }
 }
